...
function trainClassANN(topology::AbstractArray{<:Int,1}, dataset::Tuple{AbstractArray{<:Real,2}, AbstractArray{Bool,2}}; transferFunctions::AbstractArray{<:Function,1}=fill(σ, length(topology)), maxEpochs::Int=1000, minLoss::Real=0.0, learningRate::Real=0.01)
    ...
    # GPU: movemos red y datos al dispositivo (GPU si está disponible, CPU si no).
    # Trasponemos antes para evitar trasponer en cada época.
    ann = to_device(ann);
    inputsT  = to_device(Float32.(inputs'));
    targetsT = to_device(Float32.(targets'));
    ...
    # Convertimos a Float32 escalar para que se almacene en CPU sin coste de GPU.
    trainingLoss = Float32(loss(ann, inputsT, targetsT));
    ...
    while (numEpoch<maxEpochs) && (trainingLoss>minLoss)
        ...
        # Entrenamos 1 ciclo (los tensores ya están en el dispositivo correcto)
    ...
    # Devolvemos la RNA entrenada (la dejamos en CPU para que el código posterior
    # que llame a predict / classifyOutputs reciba arrays normales).
    return (cpu(ann), trainingLosses);
end;
...
function trainClassANN(topology::AbstractArray{<:Int,1},
    ...
    # GPU: movemos red y datos al dispositivo (GPU si está disponible, CPU si no).
    # Trasponemos antes para no trasponer en cada época. Los conjuntos vacíos los
    # mantenemos en CPU porque to_device(matriz_vacía) no aporta nada.
    ann = to_device(ann);
    trainingInputsT    = to_device(Float32.(trainingInputs'));
    trainingTargetsT   = to_device(Float32.(trainingTargets'));
    validationInputsT  = isempty(validationInputs)  ? validationInputs'  : to_device(Float32.(validationInputs'));
    validationTargetsT = isempty(validationTargets) ? validationTargets' : to_device(Float32.(validationTargets'));
    testInputsT        = isempty(testInputs)        ? testInputs'        : to_device(Float32.(testInputs'));
    testTargetsT       = isempty(testTargets)       ? testTargets'       : to_device(Float32.(testTargets'));
    ...
    function calculateLossValues()
        trainingLoss = Float32(loss(ann, trainingInputsT, trainingTargetsT));
        validationLoss = NaN; testLoss = NaN;
        push!(trainingLosses, trainingLoss);
        !isempty(validationInputs) && (validationLoss = Float32(loss(ann, validationInputsT, validationTargetsT)); push!(validationLosses, validationLoss);)
        !isempty(      testInputs) && (testLoss       = Float32(loss(ann,       testInputsT,       testTargetsT)); push!(      testLosses,       testLoss);)
    ...
    # Cual es la mejor ann que se ha conseguido (deepcopy en GPU también funciona)
    ...
    while (numEpoch<maxEpochs) && (trainingLoss>minLoss) && (numEpochsValidation<maxEpochsVal)
        ...
        # Entrenamos 1 ciclo (los tensores ya están en el dispositivo correcto)
        Flux.train!(loss, ann, [(trainingInputsT, trainingTargetsT)], opt_state);
    ...
    # Devolvemos la mejor RNA en CPU (para que el código que llame a predict reciba
    # arrays normales sin tener que conocer CUDA).
    return (cpu(bestANN), trainingLosses, validationLosses, testLosses);
end;
